...
RSpec.describe 'acts_as_favoritor' do
  ...
  context 'without scopes' do
    ...
    it 'responds to instance methods' do
      ...
      expect(sam).to respond_to(:favorites_count)
    ...
    describe 'favorites_by_type' do
      ...
      it 'accepts AR options' do
        expect(jon.favorites_by_type('User', limit: 0)).to eq []
      end
    end
    ...
    describe 'all_favorites' do
      ...
      it 'accepts AR options' do
        expect(jon.all_favorites(limit: 0)).to eq []
      end
    end
    ...
    describe 'favorited_by_type' do
      ...
      it 'accepts AR options' do
        expect(jon.favorited_by_type('User', limit: 0)).to eq []
      end
    end

    describe 'favorited_by_type_count' do
      it 'counts the favorited objects of a given type' do
        expect(jon.favorited_by_type_count('User')).to     eq 1
        expect(jon.favorited_by_type_count('Composer')).to eq 0
        expect(sam.favorited_by_type_count('User')).to     eq 1
        expect(sam.favorited_by_type_count('Composer')).to eq 2
      end
    end
  ...
  context 'with scopes' do
    before do
      ...
      sam.favorite(beethoven, scope: [:favorite])
      sam.favorite(rossini, scope: [:favorite])
    end
    ...
    describe 'all_favorited' do
      it 'returns favorited objects' do
        ...
        expect(sam.all_favorited(scope: [:favorite])).to eq [beethoven, rossini]
      end
    end
  end
